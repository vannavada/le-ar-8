...
//   income         — REQUIRED (basis for all savings/expenses calculations)
//   savingsRate    — REQUIRED (0% → Infinity years, but blank is meaningless;
//                             the user needs a concrete number to run this calc)
//   expectedReturn — optional (blank = 0%: linear savings path, no compounding)
//   withdrawalRate — REQUIRED (denominator in target calc; blank makes no sense)
...
export function TimeToFiCalculator() {
  const [income, setIncome] = useState<number | null>(DEFAULTS.income);
  const [savingsRate, setSavingsRate] = useState<number | null>(DEFAULTS.savingsRate);
  const [expectedReturn, setExpectedReturn] = useState<number | null>(DEFAULTS.expectedReturn);
  const [withdrawalRate, setWithdrawalRate] = useState<number | null>(DEFAULTS.withdrawalRate);

  const canCompute = income !== null && savingsRate !== null && withdrawalRate !== null;
    ...
    <CalculatorShell
      ...
      inputs={
        <div className="space-y-4">
          <CalcInput id="income" label="Annual gross income" value={income} onChange={setIncome} required prefix="$" min={0} step={5000} />
          <CalcInput id="savings" label="Savings rate" value={savingsRate} onChange={setSavingsRate} required suffix="%" min={1} max={99} step={1} hint="The most important number here. Try moving it — watch the years drop." />
          <CalcInput id="return" label="Expected annual investment return" value={expectedReturn} onChange={setExpectedReturn} suffix="%" min={0} max={20} step={0.5} hint="Use a real (after-inflation) return for inflation-adjusted results." />
          <CalcInput id="withdrawal" label="Safe withdrawal rate" value={withdrawalRate} onChange={setWithdrawalRate} required suffix="%" min={1} max={10} step={0.25} hint="4% = 25× expenses. Lower = more conservative." />
      ...
      results={
        result === null ? (
          <CalcResults empty="Enter your income, savings rate, and withdrawal rate to see your FI timeline." />
        ) : (
          <CalcResults
            headline={{
